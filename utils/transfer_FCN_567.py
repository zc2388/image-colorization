...
def transfer_FCN_ResNet50():
    ...
    x = conv_block(3, [256, 256, 1024], stage=5, block='a')(x)
    x = identity_block(3, [256, 256, 1024], stage=5, block='b')(x)
    x = identity_block(3, [256, 256, 1024], stage=5, block='c')(x)
    x = identity_block(3, [256, 256, 1024], stage=5, block='d')(x)
    x = identity_block(3, [256, 256, 1024], stage=5, block='e')(x)
    x = identity_block(3, [256, 256, 1024], stage=5, block='f')(x)

    x = conv_block(3, [256, 256, 1024], stage=6, block='a')(x)
    x = identity_block(3, [256, 256, 1024], stage=6, block='b')(x)
    x = identity_block(3, [256, 256, 1024], stage=6, block='c')(x)
    x = identity_block(3, [256, 256, 1024], stage=6, block='d')(x)
    x = identity_block(3, [256, 256, 1024], stage=6, block='e')(x)
    x = identity_block(3, [256, 256, 1024], stage=6, block='f')(x)

    x = conv_block(3, [512, 512, 2048], stage=7, block='a')(x)
    x = identity_block(3, [512, 512, 2048], stage=7, block='b')(x)
    x = identity_block(3, [512, 512, 2048], stage=7, block='c')(x)
    ...
    weights_path = os.path.expanduser(os.path.join('~', '.keras/models/fcn_resnet50_weights_tf_dim_ordering_tf_kernels_zc2388.h5'))
    #transfer if weights have not been created
    if os.path.isfile(weights_path) == False:
        flattened_layers = model.layers
        index = {}
        ...
        for layer in resnet50.layers:
            ...
            if layer.name[2] == '4':
		for new_idx in (5,6):
                    new_name = layer.name[0:2]+str(new_idx) + layer.name[3:]
                    index[new_name].set_weights(weights)
                    print(new_name + ' weight set!')         
            if layer.name[3] == '4':
                for new_idx in (5,6):
                    new_name = layer.name[0:3]+str(new_idx) + layer.name[4:]
                    index[new_name].set_weights(weights)   
                    print(new_name + ' weight set!')
            if layer.name[2] == '5':
                for new_idx in (7):
