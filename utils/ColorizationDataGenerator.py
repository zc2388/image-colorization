...
class ColorizationDirectoryIterator(Iterator):
    ...
    def __init__(self, file_path, seg_data_generator,
        ...
        if target_size:
            if self.color_mode == 'rgb':
                if self.data_format == 'channels_last':
                    ...
                    self.label_shape = self.target_size + (2,)
                else:
                    self.image_shape = (3,) + self.target_size
                    self.label_shape = (2,) + self.target_size
            else:
                if self.data_format == 'channels_last':
        ...
        super(ColorizationDirectoryIterator, self).__init__(
        ...
        for i, j in enumerate(index_array):
            ...
            if self.target_size:
                if self.crop_mode != 'none':
                    ...
                    lab_rgb_image = lab2rgb(lab_rgb_image)
                    x = lab_rgb_image*255
                    y = lab_image[:,:,1:]
            ...
            #print(batch_x[i].shape, batch_y[i].shape)
...
class ColorizationDataGenerator(object):
    ...
    def flow_from_directory(self, file_path, data_dir, data_suffix,
        ...
        return ColorizationDirectoryIterator(
            file_path, self,
